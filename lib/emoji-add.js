...
class EmojiAdd {
  ...
  async upload(src) {
    ...
    if (emojiList.length > 100) {
      logger.info('Due to rate limiting we can only upload 100 per min')
    }
    console.log(emojiList.length)
    do {
      const newIndex = currentIndex + 100
      emojiResultArray = await Promise.all(
        emojiList
          .slice(currentIndex, newIndex)
          .map(emoji => this.uploadSingle(emoji)),
      );
      currentIndex = newIndex
      logger.info('Batch uploaded')
    } while (currentIndex < emojiList.length)
